...
class Metrics(object):
    ...
    def _get_open3d_ptcloud(cls, tensor):
        ...
        # 确保数据类型为float64，Open3D要求这种类型
        if tensor.dtype != np.float64:
            tensor = tensor.astype(np.float64)
        
        # 确保张量是2D的 (N, 3)
        if tensor.ndim != 2 or tensor.shape[1] != 3:
            raise ValueError(f"Expected tensor of shape (N, 3), got {tensor.shape}")
        
        # 检查是否有无效值
        if np.any(np.isnan(tensor)) or np.any(np.isinf(tensor)):
            print(f"Warning: tensor contains NaN or Inf values")
            tensor = np.nan_to_num(tensor, nan=0.0, posinf=1e10, neginf=-1e10)
