...
def dataset_builder(args, config):
    """
      ...
      config=train : { _base_: cfgs/dataset_configs/ShapeNet-55.yaml, # 配置有npy路径
            others: {subset: 'train', npoints: 1024}},  # 覆盖每个样本点数，指定为1024，使用指定train的部分数据

    """
    dataset = build_dataset_from_cfg(config._base_, config.others) # 最终返回obj_cls(cfg)，即数据集类的一个示例
...
# todo 和上面的builder传入的是同一个config，数据集传入config的是config.dataset.train，模型builder传入的config是config.model
def model_builder(config):
    model = build_model_from_cfg(config) # 同样返回obj_cls(cfg)，即模型类的一个示例
    return model
